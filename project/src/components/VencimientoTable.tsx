...
export const VencimientoTable: React.FC<{ data: Task[] }> = ({ data }) => {
  ...
  return (
    <div ref={tableRef} className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden mb-8">
      <div className="flex justify-between items-center px-6 py-4 border-b border-gray-100">
        <h3 className="text-xl font-semibold text-gray-800">Tareas del período seleccionado</h3>
        <button
              ...
              data.map((tarea) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-gray-600">{formatDate(tarea.fecha_de_creacion)}</td>
                    <td className="px-6 py-4 text-gray-600">{formatDate(tarea.fecha_de_vencimiento)}</td>
                  </tr>
                );
              })
            )}
